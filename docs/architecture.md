...
証跡はいずれもページ全体を撮影する。運用者が実送信を承認する前に見るのも、送信前レビューが判定に使うのも同じ画像であり、表示領域だけではスクロール位置によってフォームの一部しか写らないためである。`Page.getLayoutMetrics` の `cssContentSize` を読んでから `Page.captureScreenshot` を `captureBeyondViewport: true` と `clip` 付きで 1 回だけ呼ぶ。CDP の応答がメッセージ上限を超えると接続ごと閉じて run が失われるため、撮影前に出力サイズを決めておく必要がある。
...
`clip` は常に `{ x: 0, y: 0, width: cssContentSize.width, height, scale }` で、JPEG quality は 60 である。`scale` と `height` は次の順で決める。

1. 幅の上限は絶対で、`scale` は `1280 / width` を超えない。これより広い画像は運用者にもレビューにも追加の情報を与えず、ペイロードだけが増えるためである。
2. 高さの上限 4,000 px も `scale` を下げて満たすが、下限 0.5 までとする。無制限に縮小すると、ページ全体は写っても欄の文字が読めない画像になり、証跡の意味が無くなるためである。
3. 下限に達してもなお収まらない極端に長いページは、`4000 / scale` の高さで上から切る。切り捨てた分は写らない。

したがって出力画像は常に 1,280 x 4,000 px 以内に収まる。例として幅 1,920 / 高さ 9,000 px のページは `scale` 0.5・`clip` 高さ 8,000 px となり、960 x 4,000 px の画像になる（下 1,000 px は写らない）。幅 4,000 / 高さ 9,000 px のページは幅の上限が優先されて `scale` 0.32 となり、切り詰めずに全体が 1,280 x 2,880 px に収まる。

安全弁として、想定出力が 600 万ピクセルを超える場合は quality を 45 に落とす。上記の上限がある限りこの条件には達しないが、上限を引き上げた場合やページが上限で御しきれないサイズを報告した場合に効く。

`cssContentSize` が取得できない、または正の有限値でない場合と、`clip` 付き撮影が失敗した場合は、接続が閉じていない限り従来の表示領域モードへフォールバックし、固定値ログ `browser_full_page_screenshot_fallback` を出す。接続が閉じている場合は再試行の相手が無いのでそのまま失敗させる。失敗はページ内容を漏らさないよう、どの経路でも同じ不透明なエラーにする。

ページ全体の画像は表示領域だけの画像より大きく、送信前レビューのリクエスト上限 1 MiB（`MAX_REVIEW_REQUEST_BYTES`）に近づく。超過した場合は既存の規則どおり画像を外して再構成し、`submit_review_request_built` に `withImage: false` として残る。この頻度は `imageBytes` とあわせて監視する。
...
現在保存するイベントは `job.retry_scheduled`、`job.dead_lettered`、`job.redelivery_ignored`、`agent.tool_diagnostic`、`agent.run_metrics`、`evidence.intent`、`evidence.captured`、`evidence.capture_failed` である。retry イベントにはreason code、発生元、attempt、実行時間、次回配信までの遅延秒数、retry時点のProvider呼び出し累計を保存する。tool diagnosticにはturn、固定のtool名、処理stage、固定のresult codeだけを保存する。引用検証の結果は`PROHIBITION_EVIDENCE_VERIFIED` / `PROHIBITION_EVIDENCE_NOT_FOUND` / `PROHIBITION_EVIDENCE_WEAK`のresult codeで記録し、引用文そのものは保存しない。
...
Worker 側の 3 つのガード（承認記録、dry-run 完了、日次上限）は `POST /jobs` にあり、ツールを経由しない登録にも掛かる。件数の集計と行の挿入は 1 つのトランザクションではないが、実送信は運用者が 1 つのツールから流すため窓は狭く、上限を超えるには同時実行が要る。その場合も承認記録と行ごとの dry-run 確認は残る。
...
- 送信完了は、許可したrequestを観測し、日本語の送信完了表現または`thank you`が15秒以内に新たに出現した場合に確定する。期待済みGET Documentは、送信対象frameの遷移と同じframe内の完了文言を必須にする。他frameの完了文言は判定に利用しない。
- この確認待ちの間、送信後本文の読み取りが遷移中のCDP failure（node detached、context destroyed、frame not found、target navigatedなど）で失敗した場合は、その場で`uncertain`にせず1秒後の次のポーリングで読み直し、`browser_confirmation_read_retry`（`method`と`kind`の固定値だけ）を1行出す。締切を過ぎた最後の読み取りまで同じ失敗が続いた場合だけ`SUBMIT_READ_AFTER_TEXT`とし、従来どおり`SUBMIT_RESULT_UNKNOWN`で終わる。個々のbody nodeだけが読み取り中に失われた場合も、そのbodyを完了文言なしとして数え、件数だけを`browser_confirmation_snapshot`の`staleBodyCount`に出す。2026-09-04の管理下`external-iframe`シナリオと実サイト1件で、送信直後の遷移中の読み取り失敗が即座に`SUBMIT_RESULT_UNKNOWN`になったためである。
...
- `pageText` はトップフレームの `document.body.innerText` だけであり、iframe 内の本文は含まない。したがって iframe 内にしか無い拒否文は、ページ本文の固定パターン判定でも引用検証でも扱えない。引用検証は引用がトップフレーム本文に実在することを条件にするため、iframe 内の文を引用しても受理されない。form 近傍テキストの収集は iframe 親ページ側の近接要素までは辿るが、iframe 内部の注意書きは対象外である。この場合に残る防御は送信直前の独立レビューだけであり、レビューはモデル判断なので完全ではない。
...
- Workers Logs を有効にし、invocation ごとの `outcome`、`cpuTime`、`wallTime`、`console` 出力を head sampling 100% で記録する。保持期間は Paid プランで 7 日であり、`outcome = exceededCpu` の調査は Cloudflare ダッシュボードの Workers Logs で 7 日以内に行う。ログに値、URL、自由文、session id を出さない方針は変わらない。
...
- Cloudflare の CPU 上限は、Free プランが公称 10 ms/呼び出し（実測では数百 ms まで通ることが多いが、負荷時に isolate ごと強制終了される）、Paid プランが既定 30 秒で `limits.cpu_ms` により最大 5 分まで変更できる。現在は Paid プランで、`limits.cpu_ms` は設定していない（Free プラン時は deploy が code 100328 で拒否されていた）。2026-09-03 の `exceededCpu` は報告 `cpuTime` が 165 ms、`wallTime` が 91 秒であり、CPU 総量では説明できない。Paid 移行と再 deploy の後は 9 呼び出しで 0 件、6 シナリオ 6 / 6 合格となったため、原因は Free プランの負荷時の isolate 強制終了と推定する。「上限引き上げでは解消しない可能性が高い」という従来の分析は、この結果で更新した。確定ではなく、23 シナリオ 2 巡で再発しないことの確認が残っている。再発時は Workers Logs で追跡する。
...
- `agent.run_metrics` の `durationMs` と Workers Logs の `wallTime` を突き合わせられるようになった。`outcome: exceededCpu` の原因は Free プランの負荷時の isolate 強制終了と推定しており、Paid 移行後の 23 シナリオ 2 巡で再発しないことの確認が残る。
...
- 送信前 / 送信後 / 禁止判定時の 3 段階でスクリーンショットを撮影し、Cloudflare R2 へ sha256 付きで保存し、D1 の `events` から参照する。証跡はいずれもページ全体を 1,280 x 4,000 px 以内へ縮小して撮影する。dry-run の `submit` 経路では、送信前レビューへ渡した画像そのものと各欄の値を `dry_run_before_submit` / `dry_run_field_map` として同じ仕組みで保存する。保存期間は運用ポリシー確定まで無期限とする。
